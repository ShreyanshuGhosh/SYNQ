...
from app.workers.cost_meter import trigger_meter_usage
...
async def send_message(
    ...
    async def event_source() -> AsyncIterator[dict[str, str]]:
        ...
        # Phase 5 — run_with_fallback handles retry/backoff/provider
        # switching/circuit-breaker integration. It yields ("kind",
        # payload) tuples we translate to SSE events.
        async for kind, payload in run_with_fallback(
            ...
            if kind == "event":
                ev = payload
                if ev.type == "text" and isinstance(ev.content, str):
                    buffered.append(ev.content)
                    yield _sse("token", {"text": ev.content})
                elif ev.type == "stop":
                    usage = ev.usage
                elif ev.type == "error":
                    errored = str(ev.content)
                    yield _sse("error", {"message": errored})
            elif kind == "model_switch":
                yield _sse("model_switch", payload)
            elif kind == "context_warning":
                yield _sse("context_warning", payload)
            elif kind == "provider_switched":
                # Phase 5 — surface the fallback banner BEFORE the next
                # provider's first token. The UI renders as a yellow
                # dismissible banner above the response.
                final_model = payload.get("to", final_model)
                yield _sse("provider_switched", payload)
            elif kind == "refusal":
                yield _sse("refusal", payload)
                errored = "content_filter_refusal"
            elif kind == "auth_failed":
                yield _sse("auth_failed", payload)
                errored = "auth_failed"
            elif kind == "hard_limit":
                yield _sse("hard_limit", payload)
                errored = "hard_limit_exceeded"
            elif kind == "attempt_summary":
                attempt_summary = payload
        ...
        async with _writer_session() as ws:
            assistant = Message(
                ...
                token_counts=token_counts_from_usage(final_model, usage),
            )
            ws.add(assistant)
            conv_row = await ws.get(Conversation, conv_id_local)
            if conv_row is not None:
                conv_row.version = (conv_row.version or 0) + 1
                # On a fallback, persist what we actually used so the
                # next turn's identity-drift detection has the truth.
                conv_row.current_model = final_model
            await ws.commit()
        ...
        # Persist the user-message token count off the hot path.
        # Done AFTER the stream completes so the token-counter network
        # call never blocks the visible assistant reply.
        try:
            from app.adapters import adapter_for as _adapter_for
            final_adapter = _adapter_for(final_model)
            asyncio.create_task(
                _cache_user_token_count(
                    user_msg.id, history[-1], final_adapter, final_adapter.provider
                )
            )
        except Exception:
            logger.exception("token-cache post-dispatch failed")
        ...
        # Phase 5 — fire the cost meter. Idempotent; safe to retry.
        if attempt_summary is not None:
            trigger_meter_usage(
                assistant_id,
                was_fallback=attempt_summary.was_fallback,
                fallback_reason=attempt_summary.fallback_reason,
                fallback_from=attempt_summary.fallback_from,
                compression_used=attempt_summary.compression_used,
                rag_chunks_retrieved=attempt_summary.rag_chunks_retrieved,
                latency_ms=attempt_summary.latency_ms,
            )
        else:
            trigger_meter_usage(assistant_id)
